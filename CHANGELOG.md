...
* **A settings file that is not a JSON object stopped startup**: if `~/.moleditpy/settings.json` held valid JSON that is not an object (for example `[]` or `null`), the app exited before opening a window. It now starts with the default logging settings.
* **`--install-plugin` without a terminal**: when stdin is piped or closed, the confirmation prompt ended in a traceback. The install is now aborted cleanly.
* **Opening Settings lowered some 3D values**: slider-backed values such as a bond radius of 0.29 or a light intensity of 0.57 were read back one step lower (0.28, 0.56) each time the Settings dialog was opened and closed with OK or Apply. Values now round-trip unchanged.
* **About dialog easter egg ignored Cancel**: right-clicking the About icon asks about unsaved changes before loading its demo molecule, but choosing Cancel still added the molecule to the open drawing and converted it to 3D. Cancel now leaves the document alone.
* **Molecule Analysis formula for XYZ files**: for a structure loaded from XYZ, the formula used a fixed element order (C, H, N, O, P, S, F, Cl, Br, I), so chloroacetonitrile read C2H2NCl instead of the C2H2ClN shown for the same molecule from a MOL file. It now uses RDKit's order: C, then H, then the rest alphabetically.
