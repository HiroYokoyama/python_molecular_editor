...
### SettingsDialog (`modules.settings_dialog`)
...
### AboutDialog (`modules.about_dialog`)
Displays application version, author, license, and logo.

### PeriodicTableDialog (`modules.periodic_table_dialog`)
...
### TranslationDialog (`modules.translation_dialog`)
...
### MoveGroupDialog (`modules.move_group_dialog`)
...
### AlignPlaneDialog (`modules.align_plane_dialog`)
...
### AlignmentDialog (`modules.alignment_dialog`)
...
### BondLengthDialog (`modules.bond_length_dialog`)
...
### AngleDialog (`modules.angle_dialog`)
...
### DihedralDialog (`modules.dihedral_dialog`)
...
### AnalysisWindow (`modules.analysis_window`)
...
### ConstrainedOptimizationDialog (`modules.constrained_optimization_dialog`)
...
### CalculationWorker (`modules.calculation_worker`)
...
### PluginManagerWindow (`modules.plugin_manager_window`)
...
### ColorSettingsDialog (`modules.color_settings_dialog`)
...
### UserTemplateDialog (`modules.user_template_dialog`)
...
### MoleculeScene (`modules.molecule_scene`)
...
### ZoomableView (`modules.zoomable_view`)
...
### AtomItem / BondItem (`modules.atom_item`, `modules.bond_item`)
...
### CustomQtInteractor (`modules.custom_qt_interactor`)
A QWidget wrapper for the VTK render window.
- **Role**: Embeds the VTK 3D view into the PyQt application.

### CustomInteractorStyle (`modules.custom_interactor_style`)
...
### Dialog3DPickingMixin (`modules.dialog3_d_picking_mixin`)
