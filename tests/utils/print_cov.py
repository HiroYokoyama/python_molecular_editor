...
subprocess.run(
    [sys.executable, "-m", "coverage", "json", "--rcfile", cvrc, "-o", os.path.join(BASE, "cov_core.json")],
    env=env, cwd=ROOT, stdout=devnull, stderr=devnull, check=True
)
...
try:
    subprocess.run(
        [sys.executable, "-m", "coverage", "json", "--rcfile", tmp_rc, "-o", os.path.join(BASE, "cov_full.json")],
        env=env, cwd=ROOT, stdout=devnull, stderr=devnull, check=True
    )
finally:
...
subprocess.run(
    [sys.executable, "-m", "coverage", "html", "--rcfile", cvrc, "-d", os.path.join(BASE, "coverage_html")],
...
def build_table(files, totals, title):
    ...
    for fname, info in sorted(files.items()):
        s = info["summary"]
        if s["num_statements"] == 0: continue
    ...
    for d in sorted(files_by_dir.keys()):
        for fname, s in files_by_dir[d]:
            p = s["percent_covered"]
            ml = s["missing_lines"]
            lines.append(f"| {fname:<55} | {s['num_statements']:>6} | {ml:>6} | {p:>6.1f}% |")

    lines.append(f"| **TOTAL** | **{totals['num_statements']}** | **{totals['missing_lines']}** | **{totals['percent_covered']:.2f}%** |")
    lines.append("")
    return lines
...
markdown_lines.append(f"- **Overall Project Coverage (Full)**: **{full_totals['percent_covered']:.2f}%**")
markdown_lines.append(f"- **Core Molecular Logic Coverage**: **{core_totals['percent_covered']:.2f}%**")
markdown_lines.append("")
markdown_lines.append("> [!NOTE]")
markdown_lines.append("> **Core Molecular Logic Coverage** excludes UI boilerplate (dialogs, view managers, and interactor styles) to focus on scientific algorithm reliability.")
markdown_lines.append("")

markdown_lines.extend(build_table(core_files, core_totals, "Core Logic Breakdown"))
markdown_lines.extend(build_table(full_files, full_totals, "Full Application Breakdown"))
...
subprocess.run([sys.executable, os.path.join(UTILS, "generate_assertion_catalog.py")], cwd=ROOT)
