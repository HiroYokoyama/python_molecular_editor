...
class DummyEditActions(MainWindowEditActions):
    ...
    def update_edit_menu_actions(self): pass
...
def test_rotate_molecule_2d_basic(mock_parser_host):
    ...
    with patch('moleditpy.modules.main_window_edit_actions.sip_isdeleted_safe', return_value=False):
...
def test_resolve_overlapping_groups_basic(mock_parser_host):
    ...
    aid2 = mock_parser_host.scene.create_atom("O", QPointF(0, 0.1)) # Within 0.5 threshold
    a2 = mock_parser_host.data.atoms[aid2]['item']
