...
from PyQt6.QtWidgets import QGraphicsScene, QStyleOptionGraphicsItem, QGraphicsItem, QGraphicsSceneHoverEvent
...
def test_atom_item_paint_resilience_to_deleted_bond(mock_parser_host):
    ...
    type(deleted_bond).atom1 = property(lambda self: (_ for _ in ()).throw(RuntimeError("C++ object deleted")))
...
def test_bond_double_non_ring_parallel(mock_parser_host):
    ...
    assert painter.drawLine.call_count == 2, "Non-ring double bond should draw 2 parallel lines"
...
def test_bond_hover_effects(mock_parser_host):
    ...
    with patch('PyQt6.QtWidgets.QGraphicsItem.hoverEnterEvent'), \
