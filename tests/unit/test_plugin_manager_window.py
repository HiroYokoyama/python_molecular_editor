...
@patch("os.remove")
def test_on_remove_plugin_single_file(mock_remove, mock_exists, mock_info, mock_question, mock_plugin_manager, qtbot):
...
@patch("os.path.exists", return_value=True)
@patch("shutil.rmtree")
def test_on_remove_plugin_package(mock_rmtree, mock_exists, mock_info, mock_question, mock_plugin_manager, qtbot):
    mock_question.return_value = QMessageBox.StandardButton.Yes
    window = PluginManagerWindow(mock_plugin_manager)
    qtbot.addWidget(window)
...
def test_on_remove_plugin_error(mock_remove, mock_exists, mock_critical, mock_question, mock_plugin_manager, qtbot):
    mock_question.return_value = QMessageBox.StandardButton.Yes
    window = PluginManagerWindow(mock_plugin_manager)
    qtbot.addWidget(window)
...
def test_on_remove_plugin_not_exists(mock_exists, mock_warn, mock_plugin_manager, qtbot):
    window = PluginManagerWindow(mock_plugin_manager)
    qtbot.addWidget(window)
...
@patch("moleditpy.plugins.plugin_manager_window.QMessageBox.question")
@patch("moleditpy.plugins.plugin_manager_window.QMessageBox.information")
def test_drop_event_valid_files(mock_info, mock_question, mock_exists, mock_isdir, mock_isfile, mock_plugin_manager, qtbot):
    window = PluginManagerWindow(mock_plugin_manager)
    qtbot.addWidget(window)
    window._compute_sha256 = MagicMock(return_value="abc")
    
    mock_plugin_manager.get_plugin_info_safe.return_value = {
        "name": "Dropped", "version": "1.0", "author": "me", "description": "desc"
    }
...
@patch("moleditpy.plugins.plugin_manager_window.QMessageBox.question")
@patch("moleditpy.plugins.plugin_manager_window.QMessageBox.information")
def test_drop_event_init_py_package(mock_info, mock_question, mock_exists, mock_isdir, mock_isfile, mock_plugin_manager, qtbot):
    window = PluginManagerWindow(mock_plugin_manager)
    qtbot.addWidget(window)
    window._compute_sha256 = MagicMock(return_value="abc")
    
    mock_plugin_manager.get_plugin_info_safe.return_value = {
        "name": "Pkg", "version": "1.0", "author": "me", "description": ""
    }
    ...
    def isfile_side(x): return True
    def isdir_side(x): return False
    def exists_side(x): return True
...
@patch("moleditpy.plugins.plugin_manager_window.QMessageBox.question")
@patch("moleditpy.plugins.plugin_manager_window.QMessageBox.information")
def test_drop_event_zip_file(mock_info, mock_question, mock_isdir, mock_isfile, mock_plugin_manager, qtbot):
    window = PluginManagerWindow(mock_plugin_manager)
    qtbot.addWidget(window)
    window._compute_sha256 = MagicMock(return_value="abc")
...
@patch("moleditpy.plugins.plugin_manager_window.QMessageBox.question")
@patch("moleditpy.plugins.plugin_manager_window.QMessageBox.information")
def test_drop_event_pure_folder(mock_info, mock_question, mock_isfile, mock_isdir, mock_plugin_manager, qtbot):
    window = PluginManagerWindow(mock_plugin_manager)
    qtbot.addWidget(window)
    window._compute_sha256 = MagicMock(return_value="abc")
...
def test_compute_sha256(mock_plugin_manager, tmpdir):
    window = PluginManagerWindow(mock_plugin_manager)
    
    # test dict / neither file nor dir
    assert window._compute_sha256("/non/existent/path") == "N/A"
    
    # file
    f = tmpdir.join("test.txt")
    f.write("hello")
    sha = window._compute_sha256(str(f))
    assert sha != "N/A"
    ...
    f2 = d.join("in.txt")
    f2.write("data")
    sha_dir = window._compute_sha256(str(d))
    assert sha_dir != "N/A"
