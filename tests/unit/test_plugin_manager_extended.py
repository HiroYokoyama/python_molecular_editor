...
"""
    ...
    @patch("os.path.isdir")
    def test_install_plugin_folder(self, mock_isdir, mock_exists, mock_remove, mock_copytree, mock_rmtree):
        pm = PluginManager()
        pm.ensure_plugin_dir = MagicMock()
    ...
    @patch("shutil.copy2")
    @patch("shutil.rmtree")
    def test_install_plugin_file_existing_dir(self, mock_rmtree, mock_copy2, mock_exists, mock_isdir):
        pm = PluginManager()
        pm.ensure_plugin_dir = MagicMock()
    ...
    def test_invoke_document_reset_handlers(self):
        pm = PluginManager()
        def bad_cb(): raise RuntimeError("Reset err")
        ...
        with patch("ast.parse") as mock_parse:
            ...
            class FakeNode:
                def __init__(self):
                    ...
                    self.value = ast.Tuple(elts=[ast.Name(id='x', ctx=ast.Load())], ctx=ast.Load())
