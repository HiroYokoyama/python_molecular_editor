...
# 1. Path setup
src_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'moleditpy', 'src'))
...
class DummyAxesWidget:
    def SetOrientationMarker(self, marker): pass
    def SetInteractor(self, interactor): pass
...
def window(app, qtbot, monkeypatch):
    ...
    monkeypatch.setattr('moleditpy.modules.main_window.MainWindow.apply_initial_settings', lambda *a, **k: None)
    monkeypatch.setattr('moleditpy.modules.main_window_view_3d.MainWindowView3d.apply_3d_settings', lambda *a, **k: None)
    ...
    monkeypatch.setattr('moleditpy.modules.main_window_compute.MainWindowCompute.trigger_conversion', sync_trigger_conversion)
