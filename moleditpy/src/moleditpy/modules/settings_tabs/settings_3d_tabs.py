...
from PyQt6.QtWidgets import (
    ...
    QFormLayout, QHBoxLayout, QLabel, QWidget, QColorDialog, QFrame
)
...
class Settings3DSceneTab(SettingsTabBase):
    ...
    def _update_color_button(self):
        self.bg_button.setStyleSheet(f"background-color: {self.current_bg_color}; border: 1px solid #888;")
    ...
    def update_ui(self, settings_dict):
        ...
        if idx >= 0: self.projection_combo.setCurrentIndex(idx)
...
class SettingsModelTab(SettingsTabBase):
    ...
    def _setup_ui(self):
        ...
        if self.prefix in ["ball_stick", "cpk"]:
            self.atom_scale_slider, self.atom_scale_label = self._create_slider(50, 200, 100)
            form_layout.addRow("Atom Size Scale:", self._wrap(self.atom_scale_slider, self.atom_scale_label))
        ...
        if self.prefix in ["ball_stick", "wireframe", "stick"]:
            ...
            self.bond_radius_slider, self.bond_radius_label = self._create_slider(r_min, r_max, 100)
            form_layout.addRow("Bond Radius:", self._wrap(self.bond_radius_slider, self.bond_radius_label))

        # Model-specific multi-bond offsets
        if self.prefix in ["ball_stick", "wireframe", "stick"]:
            form_layout.addRow(self._create_separator())
            self.db_offset_slider, self.db_offset_label = self._create_slider(50, 400, 100)
            form_layout.addRow("Double Bond Offset:", self._wrap(self.db_offset_slider, self.db_offset_label))
            
            self.tr_offset_slider, self.tr_offset_label = self._create_slider(50, 400, 100)
            form_layout.addRow("Triple Bond Offset:", self._wrap(self.tr_offset_slider, self.tr_offset_label))
            
            self.db_radius_slider, self.db_radius_label = self._create_slider(20, 100, 100)
            form_layout.addRow("Double Bond Thickness:", self._wrap(self.db_radius_slider, self.db_radius_label))
            
            self.tr_radius_slider, self.tr_radius_label = self._create_slider(20, 100, 100)
            form_layout.addRow("Triple Bond Thickness:", self._wrap(self.tr_radius_slider, self.tr_radius_label))

        # Common Resolution
        form_layout.addRow(self._create_separator())
        res_ranges = {"ball_stick": (6, 32), "cpk": (8, 64), "wireframe": (4, 16), "stick": (6, 32)}
        r_min, r_max = res_ranges[self.prefix]
        self.res_slider, self.res_label = self._create_slider(r_min, r_max, 1, is_int=True)
        form_layout.addRow("Resolution (Quality):", self._wrap(self.res_slider, self.res_label))
    ...
    def _pick_bond_color(self):
        ...
        if color.isValid():
            ...
            self.bond_color_button.setStyleSheet(f"background-color: {self.current_bond_color}; border: 1px solid #888;")
    ...
    def update_ui(self, settings_dict):
        ...
        if p in ["ball_stick", "wireframe", "stick"]:
            ...
            self.db_offset_slider.setValue(int(settings_dict.get(f"{p}_double_bond_offset_factor", 2.0) * 100))
            self.tr_offset_slider.setValue(int(settings_dict.get(f"{p}_triple_bond_offset_factor", 2.0) * 100))
            self.db_radius_slider.setValue(int(settings_dict.get(f"{p}_double_bond_radius_factor", 0.8) * 100))
            self.tr_radius_slider.setValue(int(settings_dict.get(f"{p}_triple_bond_radius_factor", 0.75) * 100))

        self.res_slider.setValue(int(settings_dict.get(f"{p}_resolution", 16)))
        
        if p == "ball_stick":
            self.current_bond_color = settings_dict.get("ball_stick_bond_color", "#7F7F7F")
            self.bond_color_button.setStyleSheet(f"background-color: {self.current_bond_color}; border: 1px solid #888;")
            self.use_cpk_checkbox.setChecked(settings_dict.get("ball_stick_use_cpk_bond_color", False))
