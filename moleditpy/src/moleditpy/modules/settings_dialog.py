...
class SettingsDialog(QDialog):
    ...
    def _setup_ui(self, current_settings):
        ...
        self.tab_bs = SettingsModelTab("ball_stick", "Ball & Stick model shows atoms as spheres and bonds as cylinders.", self.default_settings, self)
        self.tab_cpk = SettingsModelTab("cpk", "CPK model shows atoms as space-filling spheres using van der Waals radii.", self.default_settings, self)
        self.tab_wf = SettingsModelTab("wireframe", "Wireframe model shows molecular structure with thin lines only.", self.default_settings, self)
        self.tab_stick = SettingsModelTab("stick", "Stick model shows bonds as thick cylinders with atoms as small spheres.", self.default_settings, self)
    ...
    def reset_current_tab(self):
        ...
        QMessageBox.information(self, "Reset Complete", f"Settings for '{self.tab_widget.tabText(self.tab_widget.currentIndex())}' tab have been reset to defaults.")

    def reset_all_settings(self):
        reply = QMessageBox.question(self, "Reset All Settings", "Are you sure you want to reset all settings to defaults?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
    ...
    def apply_settings(self):
        ...
        if scene:
            ...
            for item in scene.items():
                ...
                elif hasattr(item, "update"): item.update()
