...
if OBABEL_AVAILABLE: 
    ...
    with contextlib.suppress(ImportError, OSError, RuntimeError):
        ...
        for pattern in data_candidates:
            ...
            for m in matches:
                if os.path.isdir(m):
                    found_datadir = m; break
            if found_datadir: break
        
        if found_datadir: os.environ["BABEL_DATADIR"] = found_datadir
        os.environ["BABEL_LIBDIR"] = ob_base
    if not pybel: OBABEL_AVAILABLE = False
else: 
    pybel = None
...
def _adjust_collision_avoidance(rd_mol, check_halted_cb, safe_status_cb):
    ...
    try:
        ...
        for f_indices in frags:
            ...
            frag_data.append({
                "indices": f_indices, "positions": pos_np, "radii": radii_np,
                "max_radius": np.max(radii_np) if len(radii_np) > 0 else 1.5
            })

        scale, max_iters, grid_size = 1.1, 50, 5.0
        for _ in range(max_iters):
            if check_halted_cb(): raise WorkerHaltError("Halted")

            moved, grid = False, {}
            for i, fd in enumerate(frag_data):
                fd_min, fd_max = np.min(fd["positions"], axis=0), np.max(fd["positions"], axis=0)
                margin = fd["max_radius"] * scale
                fd["bbox_min"], fd["bbox_max"] = fd_min - margin, fd_max + margin
                g_min, g_max = (fd["bbox_min"] / grid_size).astype(int), (fd["bbox_max"] / grid_size).astype(int)
            ...
            for cell_indices in grid.values():
                if len(cell_indices) < 2: continue
                for idx_idx_i, i in enumerate(cell_indices):
                    for j in cell_indices[idx_idx_i + 1:]:
                        pair = tuple(sorted((i, j)))
                        if pair in processed_pairs: continue
                        processed_pairs.add(pair)
                        if np.any(frag_data[i]["bbox_min"] > frag_data[j]["bbox_max"]) or np.any(frag_data[j]["bbox_min"] > frag_data[i]["bbox_max"]):
                            continue
                        ...
                        for p_idx_i, p_i in enumerate(fd_i["positions"]):
                            ...
                            for p_idx_j, p_j in enumerate(fd_j["positions"]):
                                ...
                                if dist_sq < 0.0001:
                                    diff = np.random.uniform(-0.1, 0.1, 3); dist_sq = np.dot(diff, diff)
                                if dist_sq < min_dist * min_dist:
                                    dist = np.sqrt(dist_sq) or 0.0001
                                    vec = (diff / dist) * ((min_dist - dist) / 2.0)
                                    push_i += vec; push_j -= vec; collision_count += 1
            ...
            for i, push in enumerate(all_push_vectors):
                if np.any(push):
                    ...
                    for p_idx, global_idx in enumerate(frag_data[i]["indices"]):
                        conf.SetAtomPosition(global_idx, frag_data[i]["positions"][p_idx])
...
def _iterative_optimize(mol, method, check_halted_cb, safe_status_cb, max_iters=4000, chunk_size=100, options=None):
    ...
    try:
        ignore_interfrag = not (options or {}).get("optimize_intermolecular_interaction_rdkit", True)
        ff = None
        if method == "UFF":
...
def _iterative_optimize_obabel(mol, method, check_halted_cb, safe_status_cb, max_iters=4000, chunk_size=100, options=None):
    ...
    try:
        ...
        if not ff: raise ValueError(f"Forcefield '{method}' not found.")
            
        ob_mol = pybel.readstring("mol", Chem.MolToMolBlock(mol))
        if not ff.Setup(ob_mol.OBMol): raise RuntimeError(f"Failed to load parameters for '{method}'.")
    ...
    with contextlib.suppress(IndexError, ValueError, AttributeError):
        ...
        if counts_idx is not None:
            ...
            for j in range(min(nbonds, max(0, len(lines) - bond_start))):
                ...
                stereo_flag = 1 if stereo_raw == 1 else (6 if stereo_raw == 2 else stereo_raw)
            ...
            if idx is not None:
                ...
                for j in range(min(n, len(lines) - (idx + 1))):
                    ...
                    try:
                        ...
                        f = ln.split(); x, y, z = float(f[0]), float(f[1]), float(f[2])
                    parsed_coords.append((x, y, z))

    if not parsed_coords: raise ValueError("Failed to parse coordinates for direct conversion.")
                ...
                if neighs:
                    ...
                    for nbr in mol.GetAtomWithIdx(parent_idx).GetNeighbors():
                        if nbr.GetIdx() < i:
                            ...
                            vx, vy = float(p.x) - float(parent_pos.x), float(p.y) - float(parent_pos.y)
                            nrm = math.hypot(vx, vy)
                            if nrm > 1e-6: vecs.append((vx / nrm, vy / nrm))
                    
                    if vecs:
                        fx, fy = -sum(v[0] for v in vecs), -sum(v[1] for v in vecs)
                        fn = math.hypot(fx, fy)
                        if fn < 1e-6: fx, fy, fn = -vecs[0][1], vecs[0][0], math.hypot(-vecs[0][1], vecs[0][0])
                        angle = sum(1 for n in mol.GetAtomWithIdx(parent_idx).GetNeighbors() if n.GetIdx() < i and n.GetAtomicNum() == 1) * (math.pi / 6.0)
                        rx, ry = (fx/fn) * math.cos(angle) - (fy/fn) * math.sin(angle), (fx/fn) * math.sin(angle) + (fy/fn) * math.cos(angle)
                        conf.SetAtomPosition(i, rdGeometry.Point3D(float(parent_pos.x) + rx, float(parent_pos.y) + ry, 0.3))
                    else:
                        conf.SetAtomPosition(i, rdGeometry.Point3D(float(parent_pos.x) + 0.5, float(parent_pos.y) + 0.5, 0.3))
                else:
    ...
    with contextlib.suppress(AttributeError, RuntimeError, TypeError, IndexError):
        for b, e, f in stereo_dirs:
            if b < num_existing and e < num_existing:
                ...
                conf.SetAtomPosition(e, rdGeometry.Point3D(float(pos.x), float(pos.y), float(pos.z) + (1.5 if f == 1 else -1.5)))
