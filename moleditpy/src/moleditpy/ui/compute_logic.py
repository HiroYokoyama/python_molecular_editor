...
# Functional class: ComputeManager
"""
...
class ComputeManager:
    ...
    def _remove_calculating_text(self):
        ...
        if actor and hasattr(self.host.plotter, "renderer") and self.host.plotter.renderer:
    ...
    def _restore_button_ui(self):
        ...
        if hasattr(self.host, "optimize_3d_button"):
            ...
            self.host.optimize_3d_button.clicked.connect(self.host.optimize_3d_structure)
    ...
    def toggle_intermolecular_interaction_rdkit(self, checked):
        ...
        self.host.statusBar().showMessage(f"Intermolecular interaction for RDKit: {state_str}")
    ...
    def trigger_conversion(self):
        ...
        if num_frags > 1:
            self.host.statusBar().showMessage(f"Converting {num_frags} molecules to 3D with collision detection...")
        else:
        ...
        options = {
            "conversion_mode": self.host.__dict__.pop("_temp_conv_mode", self.host.settings.get("3d_conversion_mode", "fallback")),
            "optimization_method": self.host.__dict__.pop("_temp_optimization_method", None) or getattr(self.host, "optimization_method", "MMFF_RDKIT"),
            "optimize_intermolecular_interaction_rdkit": self.host.settings.get("optimize_intermolecular_interaction_rdkit", True),
            "worker_id": run_id,
        }
    ...
    def optimize_3d_structure(self):
        ...
        method = self.host.__dict__.pop("_temp_optimization_method", None) or getattr(self.host, "optimization_method", "MMFF_RDKIT")
    ...
    def _handle_chemistry_problems(self, mol, problems):
        self.host.scene.clear_all_problem_flags()
        self.host.statusBar().showMessage(f"Error: {len(problems)} chemistry problem(s) found.")
    ...
    def check_chemistry_problems_fallback(self):
        problem_atom_ids = identify_valence_problems(self.host.data.atoms, self.host.data.bonds)
        if problem_atom_ids:
            for aid in problem_atom_ids:
                ...
                if item:
                    item.has_problem = True
                    item.update()
            self.host.statusBar().showMessage(f"Error: {len(problem_atom_ids)} chemistry problems found.")
        self.host.view_2d.setFocus()
