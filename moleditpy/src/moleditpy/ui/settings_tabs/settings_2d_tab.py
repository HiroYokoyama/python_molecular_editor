...
from PyQt6.QtWidgets import (
    QPushButton, QSlider, QComboBox, QFontComboBox, 
    QFormLayout, QFrame, QHBoxLayout, QLabel, QWidget, QColorDialog
)
...
class Settings2DTab(SettingsTabBase):
    ...
    def _setup_ui(self):
        ...
        # Bond Width
        self.bond_width_2d_slider, self.bond_width_2d_label = self._create_slider_with_label(10, 200, 10.0)
        form_layout.addRow("Bond Width:", self._wrap_layout(self.bond_width_2d_slider, self.bond_width_2d_label))

        # Double Bond Spacing
        self.bond_spacing_double_2d_slider, self.bond_spacing_double_2d_label = self._create_slider_with_label(10, 200, 10.0)
        form_layout.addRow("Double Bond Spacing:", self._wrap_layout(self.bond_spacing_double_2d_slider, self.bond_spacing_double_2d_label))

        # Triple Bond Spacing
        self.bond_spacing_triple_2d_slider, self.bond_spacing_triple_2d_label = self._create_slider_with_label(10, 200, 10.0)
        form_layout.addRow("Triple Bond Spacing:", self._wrap_layout(self.bond_spacing_triple_2d_slider, self.bond_spacing_triple_2d_label))
        ...
        # Wedge Bond Width
        self.bond_wedge_width_2d_slider, self.bond_wedge_width_2d_label = self._create_slider_with_label(10, 300, 10.0)
        form_layout.addRow("Wedge Bond Width:", self._wrap_layout(self.bond_wedge_width_2d_slider, self.bond_wedge_width_2d_label))

        # Dash Count
        self.bond_dash_count_2d_slider, self.bond_dash_count_2d_label = self._create_slider_with_label(3, 20, 1.0, is_int=True)
        form_layout.addRow("Dash Count:", self._wrap_layout(self.bond_dash_count_2d_slider, self.bond_dash_count_2d_label))
        ...
        self.atom_font_size_2d_slider, self.atom_font_size_2d_label = self._create_slider_with_label(8, 72, 1.0, is_int=True)
        form_layout.addRow("Atom Label Font Size:", self._wrap_layout(self.atom_font_size_2d_slider, self.atom_font_size_2d_label))
    ...
    def update_ui(self, settings_dict):
        ...
        self.bond_width_2d_slider.setValue(int(settings_dict.get("bond_width_2d", 2.0) * 10))
        self.bond_spacing_double_2d_slider.setValue(int(settings_dict.get("bond_spacing_double_2d", 3.5) * 10))
        self.bond_spacing_triple_2d_slider.setValue(int(settings_dict.get("bond_spacing_triple_2d", 3.5) * 10))
        
        cap_style = settings_dict.get("bond_cap_style_2d", "Round")
        idx = self.bond_cap_style_2d_combo.findText(cap_style)
        if idx >= 0: self.bond_cap_style_2d_combo.setCurrentIndex(idx)

        self.bond_wedge_width_2d_slider.setValue(int(settings_dict.get("bond_wedge_width_2d", 6.0) * 10))
        self.bond_dash_count_2d_slider.setValue(settings_dict.get("bond_dash_count_2d", 8))
        
        font_family = settings_dict.get("atom_font_family_2d", "Arial")
        self.atom_font_family_2d_combo.setCurrentFont(QFont(font_family))
        
        self.atom_font_size_2d_slider.setValue(settings_dict.get("atom_font_size_2d", 20))
        self.atom_use_bond_color_2d_checkbox.setChecked(settings_dict.get("atom_use_bond_color_2d", False))
