...
class MainWindow(
    ...
    def __getattr__(self, name):
        ...
        if name in ["export_manager", "view_3d_manager", "edit_3d_manager", "edit_actions_manager"]:
            raise AttributeError(f"'{type(self).__name__}' object has no attribute '{name}'")
            
        # Core attributes that MUST be on host (skip list)
        core_attrs = {
            "settings", "plotter", "statusBar", "current_mol", "scene", 
            "view_2d", "data", "plugin_manager", "current_file_path"
        }
        if name in core_attrs or name.startswith("_"):
            raise AttributeError(f"'{type(self).__name__}' object has no attribute '{name}'")

        # Try to find the attribute in any of the injected managers
        for mgr_attr in ["export_manager", "view_3d_manager", "edit_3d_manager", "edit_actions_manager"]:
            ...
            if manager:
                ...
                if any(name in c.__dict__ for c in mgr_type.mro()) or name in manager.__dict__:
