...
# Functional class: DialogManager
"""
...
try:
    ...
    from moleditpy.ui.sip_isdeleted_safe import sip_isdeleted_safe
...
class DialogManager:
    ...
    def open_analysis_window(self):
        if self.host.current_mol:
            dialog = AnalysisWindow(
                self.host.current_mol, self.host, is_xyz_derived=self.host.is_xyz_derived
            )
            dialog.exec()
        else:
    ...
    def open_template_dialog_and_activate(self):
        ...
        def on_dialog_finished():
            if (
                ...
                self.host.scene.user_template_data = self.host._template_dialog.selected_template
    ...
    def save_2d_as_template(self):
        ...
        if not self.host.data.atoms:
            QMessageBox.warning(self.host, "Warning", "No structure to save as template.")
            return

        # Get template name
        name, ok = QInputDialog.getText(self.host, "Save Template", "Enter template name:")
        ...
        try:
            ...
            QMessageBox.critical(self.host, "Error", f"Failed to save template: {str(e)}")
    ...
    def open_translation_dialog(self):
        ...
        dialog.finished.connect(
            lambda: self.host.remove_dialog_from_list(dialog)
        )
    ...
    def open_planarize_dialog(self, plane=None):
        ...
        dialog = PlanarizeDialog(self.host.current_mol, self.host, preselected_atoms, parent=self.host)
        self.host.active_3d_dialogs.append(dialog)
        dialog.show()
        dialog.accepted.connect(
            lambda: self.host.statusBar().showMessage(
    ...
    def open_angle_dialog(self):
        ...
        dialog = AngleDialog(self.host.current_mol, self.host, preselected_atoms, parent=self.host)
        self.host.active_3d_dialogs.append(dialog)
        dialog.show()
        dialog.accepted.connect(lambda: self.host.statusBar().showMessage("Angle adjusted."))
        dialog.accepted.connect(self.host.push_undo_state)
        dialog.finished.connect(
            lambda: self.host.remove_dialog_from_list(dialog)
        )
    ...
    def open_dihedral_dialog(self):
        ...
        dialog = DihedralDialog(self.host.current_mol, self.host, preselected_atoms, parent=self.host)
        self.host.active_3d_dialogs.append(dialog)
        dialog.show()
        dialog.accepted.connect(
    ...
    def open_constrained_optimization_dialog(self):
        ...
        dialog = ConstrainedOptimizationDialog(self.host.current_mol, self.host, parent=self.host)
        self.host.active_3d_dialogs.append(dialog)
        dialog.show()
        dialog.finished.connect(lambda: self.host.remove_dialog_from_list(dialog))
