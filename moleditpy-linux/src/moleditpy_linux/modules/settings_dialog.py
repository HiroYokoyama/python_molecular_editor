...
class SettingsDialog(QDialog):
    def __init__(self, current_settings, parent=None):
        ...
        # Save reference to parent window (for Apply functionality)
        self.parent_window = parent

        # Define default settings within the class
        ...
        # --- Dedicated instance variables for managing selected colors ---
        ...
        self.aromatic_circle_checkbox.setChecked(
            current_settings.get(
                ...
                self.default_settings.get("display_aromatic_circles_3d", False),
            )
        )
        # Thickness factor is stored as a multiplier (e.g., 1.0), slider uses integer 0-300 representing 0.1x-3.0x
        thickness_factor = current_settings.get(
            "aromatic_torus_thickness_factor",
            self.default_settings.get("aromatic_torus_thickness_factor", 1.0),
        )
        ...
        self.current_bg_color_2d = current_settings.get(
            "background_color_2d", self.default_settings["background_color_2d"]
        )
        self.current_bond_color_2d = current_settings.get(
        ...
        reset_tab_button.setToolTip(
            "Reset settings for the currently selected tab only"
        )
    ...
    def create_2d_settings_tab(self):
        ...
        self.bond_width_2d_slider.valueChanged.connect(
            lambda v: self.bond_width_2d_label.setText(f"{v / 10:.1f}")
        )
        ...
        self.bond_spacing_double_2d_slider.valueChanged.connect(
            lambda v: self.bond_spacing_double_2d_label.setText(f"{v / 10:.1f}")
        )
        ...
        self.bond_spacing_triple_2d_slider.valueChanged.connect(
            lambda v: self.bond_spacing_triple_2d_label.setText(f"{v / 10:.1f}")
        )
        ...
        self.bond_wedge_width_2d_slider.valueChanged.connect(
            lambda v: self.bond_wedge_width_2d_label.setText(f"{v / 10:.1f}")
        )
        ...
        self.bond_dash_count_2d_slider.valueChanged.connect(
            lambda v: self.bond_dash_count_2d_label.setText(str(v))
        )
        ...
        self.atom_font_size_2d_slider.valueChanged.connect(
            lambda v: self.atom_font_size_2d_label.setText(str(v))
        )
    ...
    def create_scene_tab(self):
        ...
        # 4. Shininess Power (Specular Power)
        ...
        self.projection_combo.setToolTip(
            "Choose camera projection mode: Perspective (default) or Orthographic"
        )
    ...
    def create_other_tab(self):
        ...
        # Option to skip chemistry checks (moved to Other tab)
        ...
        self.kekule_3d_checkbox.setToolTip(
            "When enabled, aromatic bonds will be kekulized in the 3D view (show alternating single/double bonds)."
        )
    ...
    def create_ball_stick_tab(self):
        ...
        self.bs_atom_scale_slider.valueChanged.connect(
            lambda v: self.bs_atom_scale_label.setText(f"{v / 100:.2f}")
        )
        ...
        self.bs_bond_radius_slider.setRange(1, 50)  # 0.01 to 0.5
        self.bs_bond_radius_label = QLabel("0.10")
        self.bs_bond_radius_slider.valueChanged.connect(
            lambda v: self.bs_bond_radius_label.setText(f"{v / 100:.2f}")
        )
        ...
        # Double/Triple bond offset scale (Ball & Stick)
        ...
        self.bs_double_offset_slider.valueChanged.connect(
            lambda v: self.bs_double_offset_label.setText(f"{v / 100:.2f}")
        )
        ...
        form_layout.addRow(
            "Double Bond Offset (Ball & Stick):", bs_double_offset_layout
        )
        ...
        self.bs_triple_offset_slider.valueChanged.connect(
            lambda v: self.bs_triple_offset_label.setText(f"{v / 100:.2f}")
        )
        ...
        form_layout.addRow(
            "Triple Bond Offset (Ball & Stick):", bs_triple_offset_layout
        )
        ...
        self.bs_double_radius_slider.valueChanged.connect(
            lambda v: self.bs_double_radius_label.setText(f"{v / 100:.2f}")
        )
        ...
        form_layout.addRow(
            "Double Bond Thickness (Ball & Stick):", bs_double_radius_layout
        )
        ...
        self.bs_triple_radius_slider.valueChanged.connect(
            lambda v: self.bs_triple_radius_label.setText(f"{v / 100:.2f}")
        )
        ...
        form_layout.addRow(
            "Triple Bond Thickness (Ball & Stick):", bs_triple_radius_layout
        )
        ...
        self.bs_resolution_slider.valueChanged.connect(
            lambda v: self.bs_resolution_label.setText(str(v))
        )
        ...
        self.bs_bond_color_button.setToolTip(
            "Choose the uniform bond color for Ball & Stick model (3D)"
        )
        ...
        self.bs_use_cpk_bond_checkbox.setToolTip(
            "If checked, bonds will be colored using the atom colors (split bonds). If unchecked, a uniform color is used."
        )
    ...
    def create_cpk_tab(self):
        ...
        self.cpk_atom_scale_slider.valueChanged.connect(
            lambda v: self.cpk_atom_scale_label.setText(f"{v / 100:.2f}")
        )
        atom_scale_layout = QHBoxLayout()
        ...
        self.cpk_resolution_slider.valueChanged.connect(
            lambda v: self.cpk_resolution_label.setText(str(v))
        )
        resolution_layout = QHBoxLayout()
    ...
    def create_wireframe_tab(self):
        ...
        self.wf_bond_radius_slider.valueChanged.connect(
            lambda v: self.wf_bond_radius_label.setText(f"{v / 100:.2f}")
        )
        bond_radius_layout = QHBoxLayout()
        ...
        self.wf_double_offset_slider.valueChanged.connect(
            lambda v: self.wf_double_offset_label.setText(f"{v / 100:.2f}")
        )
        ...
        self.wf_triple_offset_slider.valueChanged.connect(
            lambda v: self.wf_triple_offset_label.setText(f"{v / 100:.2f}")
        )
        ...
        self.wf_double_radius_slider.valueChanged.connect(
            lambda v: self.wf_double_radius_label.setText(f"{v / 100:.2f}")
        )
        ...
        form_layout.addRow(
            "Double Bond Thickness (Wireframe):", wf_double_radius_layout
        )
        ...
        self.wf_triple_radius_slider.valueChanged.connect(
            lambda v: self.wf_triple_radius_label.setText(f"{v / 100:.2f}")
        )
        ...
        self.wf_resolution_slider.valueChanged.connect(
            lambda v: self.wf_resolution_label.setText(str(v))
        )
        resolution_layout = QHBoxLayout()
    ...
    def create_stick_tab(self):
        ...
        # Bond radius (same value used for atom radius)
        ...
        self.stick_bond_radius_slider.valueChanged.connect(
            lambda v: self.stick_bond_radius_label.setText(f"{v / 100:.2f}")
        )
        bond_radius_layout = QHBoxLayout()
        ...
        self.stick_double_offset_slider.valueChanged.connect(
            lambda v: self.stick_double_offset_label.setText(f"{v / 100:.2f}")
        )
        ...
        self.stick_triple_offset_slider.valueChanged.connect(
            lambda v: self.stick_triple_offset_label.setText(f"{v / 100:.2f}")
        )
        ...
        self.stick_double_radius_slider.valueChanged.connect(
            lambda v: self.stick_double_radius_label.setText(f"{v / 100:.2f}")
        )
        ...
        self.stick_triple_radius_slider.valueChanged.connect(
            lambda v: self.stick_triple_radius_label.setText(f"{v / 100:.2f}")
        )
        ...
        self.stick_resolution_slider.valueChanged.connect(
            lambda v: self.stick_resolution_label.setText(str(v))
        )
        resolution_layout = QHBoxLayout()
    ...
    def reset_current_tab(self):
        """Reset only the settings of the currently selected tab to defaults"""
        ...
        tab_settings = {
            "2D Settings": {
                ...
                "bond_spacing_double_2d": self.default_settings[
                    "bond_spacing_double_2d"
                ],
                "bond_spacing_triple_2d": self.default_settings[
                    "bond_spacing_triple_2d"
                ],
                "atom_font_size_2d": self.default_settings["atom_font_size_2d"],
                "background_color_2d": self.default_settings["background_color_2d"],
                "bond_color_2d": self.default_settings["bond_color_2d"],
                "atom_use_bond_color_2d": self.default_settings[
            ...
            "Other": {
                # other options
                "skip_chemistry_checks": self.default_settings.get(
                    "skip_chemistry_checks", False
                ),
                "display_kekule_3d": self.default_settings.get(
                    "display_kekule_3d", False
                ),
                "always_ask_charge": self.default_settings.get(
                    "always_ask_charge", False
                ),
                "display_aromatic_circles_3d": self.default_settings.get(
                    "display_aromatic_circles_3d", False
                ),
                "aromatic_torus_thickness_factor": self.default_settings.get(
                    "aromatic_torus_thickness_factor", 0.6
                ),
            },
            "Ball & Stick": {
                "ball_stick_atom_scale": self.default_settings["ball_stick_atom_scale"],
                "ball_stick_bond_radius": self.default_settings[
                    "ball_stick_bond_radius"
                ],
                "ball_stick_resolution": self.default_settings["ball_stick_resolution"],
                "ball_stick_double_bond_offset_factor": self.default_settings.get(
                    "ball_stick_double_bond_offset_factor", 2.0
                ),
                "ball_stick_triple_bond_offset_factor": self.default_settings.get(
                    "ball_stick_triple_bond_offset_factor", 2.0
                ),
                "ball_stick_double_bond_radius_factor": self.default_settings.get(
                    "ball_stick_double_bond_radius_factor", 0.8
                ),
                "ball_stick_triple_bond_radius_factor": self.default_settings.get(
                    "ball_stick_triple_bond_radius_factor", 0.75
                ),
                "ball_stick_use_cpk_bond_color": self.default_settings[
                    "ball_stick_use_cpk_bond_color"
                ],
                "ball_stick_bond_color": self.default_settings.get(
                    "ball_stick_bond_color", "#7F7F7F"
                ),
            },
            "CPK (Space-filling)": {
            ...
            "Wireframe": {
                ...
                "wireframe_double_bond_offset_factor": self.default_settings.get(
                    "wireframe_double_bond_offset_factor", 3.0
                ),
                "wireframe_triple_bond_offset_factor": self.default_settings.get(
                    "wireframe_triple_bond_offset_factor", 3.0
                ),
                "wireframe_double_bond_radius_factor": self.default_settings.get(
                    "wireframe_double_bond_radius_factor", 0.8
                ),
                "wireframe_triple_bond_radius_factor": self.default_settings.get(
                    "wireframe_triple_bond_radius_factor", 0.75
                ),
            },
            "Stick": {
                ...
                "stick_double_bond_offset_factor": self.default_settings.get(
                    "stick_double_bond_offset_factor", 1.5
                ),
                "stick_triple_bond_offset_factor": self.default_settings.get(
                    "stick_triple_bond_offset_factor", 1.0
                ),
                "stick_double_bond_radius_factor": self.default_settings.get(
                    "stick_double_bond_radius_factor", 0.6
                ),
                "stick_triple_bond_radius_factor": self.default_settings.get(
                    "stick_triple_bond_radius_factor", 0.4
                ),
            },
        }

        # Apply settings for the selected tab only
        if tab_name in tab_settings:
            ...
            # Update only the items in the selected tab with default values
            ...
            if tab_name == "2D Settings":
                self.current_bg_color_2d = updated_settings.get(
                    "background_color_2d", self.default_settings["background_color_2d"]
                )
                self.current_bond_color_2d = updated_settings.get(
                    "bond_color_2d", self.default_settings["bond_color_2d"]
                )
                self.update_2d_color_buttons()
            ...
            QMessageBox.information(
                self,
                "Reset Complete",
                f"Settings for '{tab_name}' tab have been reset to defaults.",
            )
        else:
    ...
    def get_current_ui_settings(self):
        """Get settings values from the current UI"""
        return {
            ...
            "ball_stick_double_bond_offset_factor": self.bs_double_offset_slider.value()
            / 100.0,
            "ball_stick_triple_bond_offset_factor": self.bs_triple_offset_slider.value()
            / 100.0,
            "ball_stick_double_bond_radius_factor": self.bs_double_radius_slider.value()
            / 100.0,
            "ball_stick_triple_bond_radius_factor": self.bs_triple_radius_slider.value()
            / 100.0,
            "wireframe_double_bond_offset_factor": self.wf_double_offset_slider.value()
            / 100.0,
            "wireframe_triple_bond_offset_factor": self.wf_triple_offset_slider.value()
            / 100.0,
            "wireframe_double_bond_radius_factor": self.wf_double_radius_slider.value()
            / 100.0,
            "wireframe_triple_bond_radius_factor": self.wf_triple_radius_slider.value()
            / 100.0,
            "stick_double_bond_offset_factor": self.stick_double_offset_slider.value()
            / 100.0,
            "stick_triple_bond_offset_factor": self.stick_triple_offset_slider.value()
            / 100.0,
            "stick_double_bond_radius_factor": self.stick_double_radius_slider.value()
            / 100.0,
            "stick_triple_bond_radius_factor": self.stick_triple_radius_slider.value()
            / 100.0,
            # Projection mode
            ...
            "ball_stick_bond_color": getattr(
                self,
                "bs_bond_color",
                self.default_settings.get("ball_stick_bond_color"),
            ),
            "ball_stick_use_cpk_bond_color": self.bs_use_cpk_bond_checkbox.isChecked(),
        }

    def reset_to_defaults(self):
        """Reset UI to default settings (retained for backward compatibility)"""
        self.reset_all_settings()

    def select_color(self):
        """Open color picker and reflect selected color in internal variables and UI"""
        # Get current color from internal variable and initialize color picker
    ...
    def accept(self):
        ...
        # Call apply_settings to apply settings
        self.apply_settings()
        super().accept()

    def update_ui_from_settings(self, settings_dict):
        """Update UI based on settings dictionary"""
        # 1. Scene settings
        ...
        self.axes_checkbox.setChecked(
            settings_dict.get("show_3d_axes", self.default_settings["show_3d_axes"])
        )
        self.light_checkbox.setChecked(
            settings_dict.get(
                "lighting_enabled", self.default_settings["lighting_enabled"]
            )
        )

        intensity = settings_dict.get(
            "light_intensity", self.default_settings["light_intensity"]
        )
        ...
        # Ball & Stick bond color
        self.bs_bond_color = settings_dict.get(
            "ball_stick_bond_color",
            self.default_settings.get("ball_stick_bond_color", "#7F7F7F"),
        )
        ...
        self.bs_use_cpk_bond_checkbox.setChecked(
            settings_dict.get(
                "ball_stick_use_cpk_bond_color",
                self.default_settings.get("ball_stick_use_cpk_bond_color", False),
            )
        )

        # 3. CPK settings
        cpk_atom_scale = settings_dict.get(
            "cpk_atom_scale", self.default_settings["cpk_atom_scale"]
        )
        self.cpk_atom_scale_slider.setValue(int(cpk_atom_scale * 100))
        self.cpk_atom_scale_label.setText(f"{cpk_atom_scale:.2f}")

        cpk_res = settings_dict.get(
            "cpk_resolution", self.default_settings["cpk_resolution"]
        )
        ...
        wf_db_offset = settings_dict.get(
            "wireframe_double_bond_offset_factor",
            self.default_settings.get("wireframe_double_bond_offset_factor", 3.0),
        )
        self.wf_double_offset_slider.setValue(int(wf_db_offset * 100))
        self.wf_double_offset_label.setText(f"{wf_db_offset:.2f}")

        wf_tr_offset = settings_dict.get(
            "wireframe_triple_bond_offset_factor",
            self.default_settings.get("wireframe_triple_bond_offset_factor", 3.0),
        )
        self.wf_triple_offset_slider.setValue(int(wf_tr_offset * 100))
        self.wf_triple_offset_label.setText(f"{wf_tr_offset:.2f}")

        wf_db_rad = settings_dict.get(
            "wireframe_double_bond_radius_factor",
            self.default_settings.get("wireframe_double_bond_radius_factor", 0.8),
        )
        self.wf_double_radius_slider.setValue(int(wf_db_rad * 100))
        self.wf_double_radius_label.setText(f"{wf_db_rad:.2f}")

        wf_tr_rad = settings_dict.get(
            "wireframe_triple_bond_radius_factor",
            self.default_settings.get("wireframe_triple_bond_radius_factor", 0.75),
        )
        self.wf_triple_radius_slider.setValue(int(wf_tr_rad * 100))
        self.wf_triple_radius_label.setText(f"{wf_tr_rad:.2f}")

        # 5. Stick settings
        stick_bond_radius = settings_dict.get(
            "stick_bond_radius", self.default_settings["stick_bond_radius"]
        )
        ...
        stick_db_offset = settings_dict.get(
            "stick_double_bond_offset_factor",
            self.default_settings.get("stick_double_bond_offset_factor", 1.5),
        )
        ...
        stick_tr_offset = settings_dict.get(
            "stick_triple_bond_offset_factor",
            self.default_settings.get("stick_triple_bond_offset_factor", 1.0),
        )
        ...
        stick_db_rad = settings_dict.get(
            "stick_double_bond_radius_factor",
            self.default_settings.get("stick_double_bond_radius_factor", 0.6),
        )
        self.stick_double_radius_slider.setValue(int(stick_db_rad * 100))
        self.stick_double_radius_label.setText(f"{stick_db_rad:.2f}")

        stick_tr_rad = settings_dict.get(
            "stick_triple_bond_radius_factor",
            self.default_settings.get("stick_triple_bond_radius_factor", 0.4),
        )
        ...
        self.skip_chem_checks_checkbox.setChecked(
            settings_dict.get(
                ...
                self.default_settings.get("skip_chemistry_checks", False),
            )
        )
        self.kekule_3d_checkbox.setChecked(
            settings_dict.get(
                "display_kekule_3d",
                self.default_settings.get("display_kekule_3d", False),
            )
        )
        ...
        self.always_ask_charge_checkbox.setChecked(
            settings_dict.get(
                "always_ask_charge",
                self.default_settings.get("always_ask_charge", False),
            )
        )
        ...
        self.atom_use_bond_color_2d_checkbox.setChecked(
            settings_dict.get(
                "atom_use_bond_color_2d",
                self.default_settings.get("atom_use_bond_color_2d", False),
            )
        )
        ...
        # Load Bond Cap Style
        cap_style = settings_dict.get(
            "bond_cap_style_2d", self.default_settings.get("bond_cap_style_2d", "Round")
        )
        index = self.bond_cap_style_2d_combo.findText(cap_style)
        if index >= 0:
            self.bond_cap_style_2d_combo.setCurrentIndex(index)

        bww_2d = settings_dict.get(
            "bond_wedge_width_2d", self.default_settings["bond_wedge_width_2d"]
        )
        ...
        self.bond_dash_count_2d_label.setText(
            str(self.bond_dash_count_2d_slider.value())
        )
