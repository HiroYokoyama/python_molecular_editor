...
### 4.3. Using Templates
  ...
  * **Alkyl Chains:** Click the `-(-)n-` button on the template toolbar, then drag on the canvas. Every bond keeps the standard 2D bond length and the chain axis snaps to 15° steps, so the drag distance decides the chain length and the drag direction decides its orientation; the zigzag bends toward whichever side of the axis the cursor is on. The number of carbon atoms that will be added (`n = ...`) is shown next to the cursor while dragging, and the chain is only created when the mouse is released. Starting the drag on an existing atom extends that atom instead of duplicating it — the reused atom is not counted in `n`. Releasing on or near an existing atom joins the chain to it, using the same bond snapping distance as ordinary bond drawing (adjustable in `Settings`). Only the final bond takes up the slack, coming out a little longer or shorter so that every other bond keeps its exact length.
