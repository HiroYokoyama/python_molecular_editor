...
### 9.1. プラグインの管理
メニューの Plugin \> Plugin Manager... からプラグインマネージャーを開くことができます。この画面では、インストール済みのプラグインの確認、有効化/無効化の切り替え、再読み込み、および削除が可能です。

各プラグイン行の **Status** 列にはチェックボックスが用意されています：
* **プラグインの有効化・無効化:** チェックを外すことでプラグインを無効化できます。無効化されたプラグインは灰色の **Disabled** ステータスで表示され、コードの読み込みや実行は行われません。設定は `~/.moleditpy/disabled_plugins.json` に自動保存され、ダイアログを閉じた際および次回起動時に反映されます。
