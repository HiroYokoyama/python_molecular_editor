...
設定可能な項目:
  ...
  * **2D Settings タブ:**
      ...
      * **Atom Hit / Snap Settings:**
          * 原子の判定・スナップ距離 (Atom Hit / Snap Distance, px): 画面上のピクセル単位 (ズーム倍率に依存しない) で指定する 1 つの距離で、結合を描くときに既存の原子へスナップする範囲、キーボードショートカットがカーソル下の原子を直接編集する範囲、原子や結合がホバーハイライトを表示しクリックに反応する範囲をまとめて制御します。
      * **Template Settings:**
