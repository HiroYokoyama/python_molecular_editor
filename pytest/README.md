...
This is the main test file, containing all unit and GUI test cases.
  ...
  * **Test Cases List**:
      ...
      * **GUI Tests (`@pytest.mark.gui`)**:
          ...
          * `test_project_save_load_round_trip`: Tests full round-trip save and load of project data.
          * `test_file_import_smiles_error`: Tests error handling for invalid SMILES input.
          * `test_undo_redo_boundary`: Tests undo/redo resilience on empty stacks.
          * `test_import_invalid_mol_file`: Tests handling of corrupted .mol files.
          * `test_clear_2d_editor_cancel`: Verifies logic for cancelling the Clear All dialog.
          * `test_clipboard_copy_empty_selection`: Ensures safe handling of copy on empty selection.
