...
def sync_linux(dry_run=False, verbose=False):
        ...
        for file in files:
            ...
            if dest_file not in valid_dest_files:
                if not dry_run:
                    ...
                    is_pyc = dest_file.endswith(".pyc") or dest_file.endswith(".pyo") or "__pycache__" in dest_file
    ...
    print(f"{prefix}Synchronization complete! {synced_files_count} files synced/modified.")
...
if __name__ == "__main__":
    if "-h" in sys.argv or "--help" in sys.argv:
        ...
        print("  --dry, --dry-run Simulate the synchronization process without modifying any files")
        print("  -v, --verbose    Show verbose output (e.g., include .pyc files in orphan cleanup logs)")
        sys.exit(0)
