...
def create_file_icon_base_v2(filename="moleditpy_file_icon_v2.png", size_px=1024):
    ...
    shadow_coords = [(p[0] + shadow_off_x, p[1] + shadow_off_y) for p in paper_coords]
    ...
    def draw_faint_hexagons():
        ...
        for row in range(
            int((doc_y - 10) // v_step), int((doc_y + doc_h + 10) // v_step)
        ):
            for col in range(
                int((doc_x - 10) // h_step), int((doc_x + doc_w + 10) // h_step)
            ):
                x_pos = col * h_step
                y_pos = row * v_step
                if row % 2 == 1:
    ...
    fold_shadow_coords = [
        p3,
        (doc_x + doc_w - fold_size, doc_y + doc_h - fold_size),
        p4,
    ]
    ...
    bar_rect = patches.Rectangle(
        (doc_x, bar_y), doc_w, bar_h, facecolor="#0074D9", alpha=0.9, zorder=2
    )
    ...
    # 保存（余白調整）
    plt.savefig(
        filename, bbox_inches="tight", pad_inches=0.1, transparent=True, dpi=dpi
    )
    plt.close()
